...
To avoid this, you can set a data availability limit which is applied individually to each aggregation. For example, setting the limit at 0.6 would mean that a score is only calculated for units that have at least 60% data availability - if the availability is lower, for a given aggregation group, the score is treated as a missing value.
...
*Anna maybe add a pic of a table from one of your examples here with some more text?*
